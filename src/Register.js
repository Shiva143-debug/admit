...
const Register = () => {
  ...
  return (
    <div className='form-container'>
      ...
      <p>If you have already an Account? <span  onClick={onlogin} style={{color:"blue",cursor:"pointer"}}>Login</span></p>
      ...
      <input type="number" value={mobileNumber} onChange={(e) => setMobileNumber(e.target.value)} placeholder="Mobile Number" className="form-input" required />
      <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Address" className="form-input" required/>
      <button type="submit" className="form-button" disabled={loading}>{loading ? <div className="spinner"></div> : "Register"} </button>
    </form> */}
      <form onSubmit={handleRegister} className="register-form">
        <h2 className="form-title">Register</h2>
